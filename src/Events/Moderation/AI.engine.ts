...
const event: Event = {
    ...
    execute: async (message: Message, client: BaseClient) => {
        ...
        const findAttribute = (name: string, arr: IScore[]) =>
            arr.find((attr) => attr.name === name);
        ...
        const ChartConfig: ChartConfiguration = {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Score',
                        data: [INSULT_SCORE!.value, TOXICITY_SCORE!.value, PROFANITY_SCORE!.value],
                        borderRadius: 20,
                        borderWidth: 8,
                        maxBarThickness: 80,
                        borderColor: [
                ...
                Delete: new EmbedBuilder()
                    ...
                    .addFields(
                        { name: 'Message', value: `${message}`, inline: false },
                        {
                            ...
                            value: SCORES.map(
                                (score) =>
                                    `${icon.reply.default} **${score.name}**: ${score.value}`,
                            ).join('\n'),
                            inline: false,
                        },
                    )
...
async function timeoutAction(
    msg: Message,
    LogChannels: string[],
    SendData: MessageOptions,
    MemberSend: MessageOptions,
) {
    await msg.delete();
