...
const event: Event = {
	...
	execute: async (message: Message, client: BaseClient) => {
		...
		const PLow = Punishments[0];
		const PMed = Punishments[1];
		const PHigh = Punishments[2];
		...
		try {
			const isMemberAdmin = (
				await guild?.members.fetch({ user: author })
		...
		const AvgScore =
			SCORES.reduce((a, b) => a + b.value, 0) / SCORES.length;
		...
		async function analyzeMessage(message: string) {
			...
			try {
				...
				const INSULT_SCORE: number =
					speech.attributeScores.INSULT.summaryScore.value;
				const TOXICITY_SCORE: number =
					speech.attributeScores.TOXICITY.summaryScore.value;
				const PROFANITY_SCORE: number =
					speech.attributeScores.PROFANITY.summaryScore.value;
