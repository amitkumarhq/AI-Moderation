			...
			group
				...
				.addSubcommand((subcommand) =>
					subcommand
						.setName('remove')
						.setDescription(
							'Remove Users/Roles From Automod Bypass'
						)
						.addUserOption((option) =>
							option
								.setName('user')
								.setDescription(
									'Remove User From Automod Bypass'
								)
								.setRequired(false)
						)
						.addRoleOption((option) =>
							option
								.setName('role')
								.setDescription(
									'Remove Role From Automod Bypass'
								)
								.setRequired(false)
						)
				)
				.addSubcommand((subcommand) =>
					subcommand
						.setName('list')
						.setDescription('List Automod Bypass')
				)
		)
		.addSubcommandGroup((group) =>
			group
					...
					subcommand
						...
						.addChannelOption((option) =>
							option
								.setName('channel')
								.setDescription(
									'The Channel For Automod Logging'
								)
								.addChannelTypes(
						...
						case 'channel':
							{
								try {
									...
									docs.ChannelIDs.splice(
										docs.ChannelIDs.indexOf(channel?.id),
										1
									);
									await docs.save();
								...
								try {
									...
									async function FetchAndPush() {
										docs?.ChannelIDs.forEach(async (c) => {
											const channel =
												await client.channels.fetch(c);
											ChannelIDs.push(channel!);
										});

										docs?.LogChannelIDs.forEach(
											async (c) => {
												const channel =
											...
											new EmbedBuilder()
												...
												.addFields(
													...
													},
													{
														...
														value: `
															\`•\` **Low:** ${docs.Punishments[0].toUpperCase() || 'None'}
															\`•\` **Medium:** ${docs.Punishments[1].toUpperCase() || 'None'}
															\`•\` **High:** ${docs.Punishments[2].toUpperCase() || 'None'}
															ㅤ
															`,
														inline: false,
													},
													...
													},
													{
														...
														value: `
															\`•\` **Users:** ${docs.BypassUsers.length || 'None'}
